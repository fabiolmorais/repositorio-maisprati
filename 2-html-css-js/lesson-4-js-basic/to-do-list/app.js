...
class Database {
  ...
  searchTasks(searchTask) {
    ...
    let filteredTasks = tasks.filter((task) => {
      return (
        (!searchTask.year || task.year == searchTask.year) &&
        (!searchTask.month || task.month == searchTask.month) &&
        (!searchTask.day || task.day == searchTask.day) &&
        (!searchTask.type || task.type == searchTask.type) &&
        (!searchTask.description ||
          task.description
            .toLowerCase()
            .includes(searchTask.description.toLowerCase()))
      );
    });
...
function loadTasks(tasks = database.loadTasks()) {
  ...
  tasks.forEach((task) => {
    ...
    btn.id = localStorage.getItem("id");
...
function searchTasks() {
  ...
  let searchTask = new Task(year, month, day, type, description);

  let filteredTasks = database.searchTasks(searchTask);
